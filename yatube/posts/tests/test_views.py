...
CON = {
    ...
    'POSTS_EDIT_TMP': 'posts/create_post.html',
...
class PostPagesTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.user2 = User.objects.create_user(username=CON['USER_2_NAME'])
        cls.authorized_client = Client()
        cls.authorized_client.force_login(cls.user)

        cls.group = Group.objects.create(
            title=CON['GROUP_1_TITLE'],
            slug=CON['GROUP_1_SLUG'],
            description=CON['GROUP_1_DESCRIPTION'],
        )
        cls.group2 = Group.objects.create(
            title=CON['GROUP_2_TITLE'],
            slug=CON['GROUP_2_SLUG'],
            description=CON['GROUP_2_DESCRIPTION'],
        )
        ...
        for i in range(CON['LAST_POST_IND']):
            if i <= CON['NUM_DIF_POSTS'] - 1:
    ...
    def test_correct_template_in_all_views(self):
        ...
        templates = {
            reverse('posts:index'): CON['POSTS_INDEX_TMP'],
            reverse('posts:post_detail', kwargs={'post_id':
                                                 CON['LAST_POST_IND']}):
            CON['POSTS_DETAIL_TMP'],
            reverse('posts:profile', kwargs={'username': CON['USER_1_NAME']}):
            CON['POSTS_PROFILE_TMP'],
            reverse('posts:post_edit', kwargs={'post_id': CON['LAST_POST_IND']}
                    ):
            CON['POSTS_EDIT_TMP'],
            reverse('posts:post_create'): CON['POSTS_CREATE_TMP'],
            reverse('posts:group_list', kwargs={'slug': CON['GROUP_1_SLUG']}):
            CON['POSTS_GROUP_TMP']
        }
    ...
    def test_paginator_and_post_exist_on_all_pages(self):
        ...
        pages = {reverse('posts:index'): settings.POSTS_PER_PAGE,
                 ...
                 CON['LAST_POST_IND'] - settings.POSTS_PER_PAGE,
                 reverse('posts:group_list',
                         kwargs={'slug': slug}) + '?page=2':
                 CON['NUM_DIF_POSTS'],
                 reverse('posts:profile',
                         kwargs={'username': username}
                         ) + '?page=2': CON['NUM_DIF_POSTS']
                 }
        for page_adress, posts_count in pages.items():
            with self.subTest(page_adress=page_adress):
                ...
                if posts_count == settings.POSTS_PER_PAGE:
                    ...
                    self.assertEqual(self.posts[CON['LAST_POST_IND'] - 1].text,
                                     first_object.text)
                    self.assertEqual(self.group.id, first_object.group_id)
                    self.assertEqual(self.posts[CON['LAST_POST_IND'] - 1].pk,
                                     first_object.pk)
    ...
    def test_post_details_context(self):
        ...
        response = (self.authorized_client.
                    get(reverse('posts:post_detail',
                                kwargs={'post_id': CON['LAST_POST_IND']})))
        # В списке posts нумерация с 0, а pk сообщений начинается с 1.
        self.assertEqual(self.posts[CON['LAST_POST_IND'] - 1],
    ...
    def test_edit_post_context(self):
        ...
        response2 = authorized_user_2.get(reverse('posts:post_edit',
                                                  kwargs={'post_id': 16}))
        is_edit = {CON['USER_1_NAME']: response.context.get('is_edit'),
                   CON['USER_2_NAME']: response2.context
                   }
        ...
        self.assertEqual(is_edit[CON['USER_1_NAME']], True)
        self.assertIsNone(is_edit[CON['USER_2_NAME']])
    ...
    def test_create_post_with_other_group(self):
        ...
        group_slugs = (CON['GROUP_1_SLUG'], CON['GROUP_2_SLUG'])
...
class ImageInPostsTests(TestCase):
    ...
    def test_image_in_context_of_paginator(self):
        ...
        если она есть.
        """
        adresses = (CON['POSTS_INDEX_URL'],
                    CON['GROUP_1_URL'],
                    CON['POSTS_PROFILE_USER_1_URL']
                    )
    ...
    def test_image_in_db_when_postform_sent(self):
        ...
        sleep(0.0001)
        authorized_client.post(CON['POSTS_CREATE_URL'], data=form_data,
                               follow=False)
...
class CachTests(TestCase):
    ...
    def test_index_cache(self):
        response = self.guest_client.get(CON['POSTS_INDEX_URL'])
        self.post.delete()
        sleep(settings.CACHE_TIME - 1)
        response_2 = self.guest_client.get(CON['POSTS_INDEX_URL'])
        self.assertEqual(response.content, response_2.content)
        sleep(settings.CACHE_TIME + 1)
        response_3 = self.guest_client.get(CON['POSTS_INDEX_URL'])
...
class FollowTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.guest_client = Client()
        cls.author = User.objects.create_user(username=CON['USER_1_NAME'])
        cls.follower = User.objects.create_user(username=CON['USER_2_NAME'])
        cls.nonfollower = User.objects.create_user(username=CON['USER_3_NAME'])
